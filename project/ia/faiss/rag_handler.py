...
def smart_chunk_json(data, model_name, max_tokens=Config.CHUNK_SIZE):
    """
...
def smart_chunk(text, model_name=Config.RAG_MODEL, max_tokens=Config.CHUNK_SIZE):
    """
...
def smart_chunk_auto(content, filename="", model_name=Config.RAG_MODEL, max_tokens=Config.CHUNK_SIZE):
    """
...
def main():
    ...
    if all_chunks_local:
        chunks, metadata, embedder, index = faiss_index_handler(all_chunks_local, metadata_local)    
    ...
    results = retrieve("none", query)  # récupère une liste
    for r in results:
