...
const props = defineProps<{ items: WorkorderListItem[]; total: number; page: number; pageSize: number; loading: boolean }>()
...
function openWorkorder(p: WorkorderListItem){ router.push(`/workorders/${encodeURIComponent(p.id)}`) }
</script>
...
<template>
  ...
  <div class="tableWrap">
    <table class="table">
      <thead><tr><th>Workorder</th><th>Status</th><th>Created</th><th>Finished</th></tr></thead>
      <tbody>
        <tr v-if="loading"><td colspan="4" class="muted">Chargement…</td></tr>
        <tr v-for="p in items" :key="p.id" class="row" @click="openWorkorder(p)">
