...
const props = defineProps<{ items: any[]; total: number; page: number; pageSize: number; loading: boolean }>()
...
function openPart(a: any){ router.push(`/parts/${encodeURIComponent(a.part_id)}`) }


async function downloadRepport(path:String) {
 const res = await api.get('/api/download', { params: { path: path } })
}

async function load() {
  loading.value = true
  try {
    const res = await api.get('/api/anomalies', { params: { page: page.value, page_size: pageSize.value } })
...
onMounted(async () => {
  await load()
  sock = getSocket()
  sock.on('anomaly', (event_ts: any) => {
    load()
  })
})

onBeforeUnmount(() => {
  if (sock) sock.off('plcanomaly_event')
})
</script>
...
<template>
  ...
  <div class="tableWrap">
    ...
    <tbody>
      ...
      <tr v-for="a in anomalies" :key="a.id" class="row" @click="openPart(a)">
         <td>{{ a.part_id }}</td>
        <td>{{ a.event_ts }}</td>
        <td>{{ a.machine }}</td>
        <td>{{ a.cycle }}</td>
        <td>{{ a.step_id }}</td>
        <td>{{ a.rule_reasons }}</td>
        <td>  
          <span class="badge"
              :class="a.severity == 'MAJOR' 
              || 
              a.severity == 'CRITICAL' ? 'error': a.severity.toLowerCase() ">{{ a.severity }}
          </span>
        </td>
        <td @click="downloadRepport(a.report_path )">{{ a.report_path }}</td>
      </tr>
    </tbody>
    </table>
  </div>
