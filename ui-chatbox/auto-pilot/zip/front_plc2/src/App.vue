  ...
  <div class="shell">
    <header class="topbar">
      <div class="brand">
        <div class="dot"></div>
        <div>
          <div class="title">PLC MES</div>
          <div class="subtitle">Live production</div>
        </div>
      </div>

      <div class="actions">
        <span v-if="auth.isAuthenticated()" class="pill">JWT</span>
        <button v-if="auth.isAuthenticated()" class="btn" @click="logout">Logout</button>
      </div>
    </header>
...
<style scoped>
.shell{min-height:100vh;background:#0b0f17;color:#e6edf3;font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Arial}
